...
class CustomTextEncoder(torch.nn.Module):
    def __init__(self, clip_model, dtype=torch.float16, device=torch.device('cuda')):
        super().__init__()
        self.dtype = dtype
        self.device = device
    ...
    def forward(self, token_ids, token_tensors=None, enable_pos_emb=False, get_feature=False):
        ...
        x = x.permute(1, 0, 2)
        text_feats = self.transformer(x)  # (8, N, D) where T=8 is the transformer attention dimension

        x = text_feats.permute(1, 0, 2)
        x = self.ln_final(x)
        tf = (
            x[
