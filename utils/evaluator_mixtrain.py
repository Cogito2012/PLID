...
class MixTrainEvaluator(Evaluator):
    ...
    """
    def __init__(self, dset, seen_pairs):
    ...
    def generate_predictions(self, scores, bias=0.0, topk=1):  # (Batch, #pairs)
        '''
        Inputs
    ...
    def score_clf_model(self, scores, topk=1):
        '''
        ...
        # Go to CPU
        attr_pred, obj_pred = attr_pred.to('cpu'), obj_pred.to('cpu')
        ...
        results = self.generate_predictions(scores)
        results['biased_scores'] = scores

        return results

    def score_manifold_model(self, scores, bias=0.0, topk=1):
        '''
        ...
        orig_scores = scores.clone()
        results = self.generate_predictions(scores, bias, topk)
        results['scores'] = orig_scores
        return results

    def score_fast_model(self, scores, bias=0.0, topk=1, pair_truth=None):
        '''
        Wrapper function to call generate_predictions for manifold models
        '''
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk=1):
        ...
        for bias in biaslist:
            ...
            results = self.score_fast_model(scores, bias=bias, topk=topk)
